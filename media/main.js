// Call Charts expand view — runs inside the webview.
// All dynamic data (chart SVG, node metadata, legend) arrives via the DOM;
// everything interactive here talks to the extension through postMessage.
...
function callerFeedStages(startEl) {
    ...
    while (queue.length) {
        ...
        for (const edge of edgesFromParent.get(current.dataset.nodeId) ?? []) {
            ...
            if (!child || child.dataset.side !== 'caller') {
                continue;
            }
...
function startFlow(el) {
    ...
    if (el.dataset.side === 'caller') {
        ...
        for (const edge of chainToRoot(el)) {
            stages.push([edge]);
        }
    } else if (el === rootEl) {
        ...
        stages = callerFeedStages(rootEl);
        for (const edge of chainToRoot(el).reverse()) {
            stages.push([edge]);
        }
    }
    if (!stages.length) {
    ...
    const pulse = (nodeEl) => {
        ...
        setTimeout(() => {
            ...
            if (nodeEl === el) {
                nodeEl.style.filter = hoverGlow;
            } else {
                nodeEl.style.filter = activeNodeIds.has(nodeEl.dataset.nodeId) ? '' : DIM_NODE;
            }
        }, 240);
    };
    ...
    const step = (now) => {
        ...
        if (stage !== lastStage) {
            for (const packet of packets) {
                if (packet.stage === lastStage) {
                    pulse(nodeById.get(packet.edge.dataset.end));
                }
            }
            lastStage = stage;
        }
